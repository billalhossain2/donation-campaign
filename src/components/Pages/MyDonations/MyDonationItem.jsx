...
const MyDonationItem = ({donation}) => {
    const {picture, category, title, price, card_bg_color, category_bg_color, text_color} = donation || {};
  return (
    <div style={{backgroundColor:card_bg_color}} className='flex lg:flex-row flex-col items-center xl:w-auto'>
        <img className='lg:w-[300px] w-[100%]' src={picture} alt="donated image" />
        <div className='pe-5 py-3 flex items-center'>
            ...
            <button style={{backgroundColor:text_color}} className='px-5 py-2 rounded-md font-bold text-white'>View Details</button>
            </div>
        </div>
    </div>
  )
